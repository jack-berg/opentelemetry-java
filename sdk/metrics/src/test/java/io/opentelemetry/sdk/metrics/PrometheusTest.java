...
class PrometheusTest {
  ...
  void attributesUnknown() {
    ...
    Histogram nativeHistogramWithLabels =
        Histogram.builder().name("native_histogram").nativeOnly().withoutExemplars().labelNames("key").register();

    CounterDataPoint[] counterDataPoints = new CounterDataPoint[labelValues.length];
    DistributionDataPoint[] histogramDataPoints = new DistributionDataPoint[labelValues.length];
    DistributionDataPoint[] nativeHistogramDataPoints = new DistributionDataPoint[labelValues.length];
    for (int i = 0; i < labelValues.length; i++) {
